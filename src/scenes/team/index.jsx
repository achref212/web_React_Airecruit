...
const JobsManagement = () => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (response.data) {
          ...
          setNewJob(prev => ({ ...prev, recruiter_id: response.data.user._id }));
  ...
  const fetchJobs = async (userId) => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/jobs/recruiter`, {
      ...
      console.error("Failed to fetch jobs for recruiter:", error.response ? error.response.data : error.message);
    }
  };
  ...
  const handleCreateSave = async () => {
    if (!newJob.recruiter_id) {
      alert("Recruiter ID is missing. Please check if you're logged in correctly.");
      return;
    }
  ...
  const fetchJobApplications = async (jobId) => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/job-applications/${jobs[jobId]._id}`);
